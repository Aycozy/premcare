...
export default function ClinicianMessagesScreen() {
    ...
    useEffect(() => {
        if (profile) fetchConversations(profile.id, 'clinician');
    }, [profile]);
    ...
    const renderConvo = ({ item }: { item: Conversation }) => (
        <TouchableOpacity
            ...
            activeOpacity={0.8}
            onPress={() => router.push(`/(clinician)/chat/${item.patient_id}` as any)}
        >
            <View style={styles.avatar}>
                <Text style={styles.avatarText}>
                    {item.patient?.full_name?.charAt(0)?.toUpperCase() || 'P'}
                </Text>
            </View>
            <View style={styles.convoContent}>
                <View style={styles.convoHeader}>
                    <Text style={styles.convoName} numberOfLines={1}>
                        {item.patient?.full_name?.split(' ')[0] || 'Patient'}
                    </Text>
            ...
            <FlatList
                ...
                refreshControl={
                    <RefreshControl
                        ...
                        onRefresh={() => profile && fetchConversations(profile.id, 'clinician')}
...
const styles = StyleSheet.create({
    ...
    convoName: { fontSize: fontSize.base, fontWeight: fontWeight.semibold, color: colors.textPrimary, flex: 1 },
    convoTime: { fontSize: fontSize.xs, color: colors.textMuted, marginLeft: spacing.sm },
