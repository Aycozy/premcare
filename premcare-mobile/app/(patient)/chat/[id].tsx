...
export default function PatientChatScreen() {
    ...
    const { id: clinicianId } = useLocalSearchParams<{ id: string }>();
    ...
    useEffect(() => {
        const init = async () => {
            if (!profile || !clinicianId) return;
            const convoId = await getOrCreateConversation(profile.id, clinicianId);
            if (convoId) {
        ...
        <SafeAreaView style={styles.safeArea}>
            ...
            <View style={styles.header}>
                ...
                <View style={styles.headerInfo}>
                    ...
                    <View>
                        <Text style={styles.headerName}>Clinician</Text>
...
const styles = StyleSheet.create({
    ...
    messageBubbleRow: {
        flexDirection: 'row', marginBottom: spacing.sm, justifyContent: 'flex-start',
    },
    messageBubbleRowMe: { justifyContent: 'flex-end' },
    messageBubble: {
        maxWidth: '78%', borderRadius: borderRadius.lg, padding: spacing.md,
    ...
    messageText: {
        fontSize: fontSize.base, color: colors.textPrimary, lineHeight: 22,
    },
    messageTextMe: { color: colors.textPrimary },
    messageTime: {
        fontSize: 10, color: colors.textMuted, marginTop: 4, textAlign: 'right',
    },
    messageTimeMe: { color: 'rgba(255,255,255,0.6)' },
    emptyChat: {
        alignItems: 'center', justifyContent: 'center', paddingVertical: spacing['5xl'],
    },
    emptyChatText: {
        fontSize: fontSize.sm, color: colors.textMuted, marginTop: spacing.md, textAlign: 'center',
    },
    inputBar: {
        flexDirection: 'row', alignItems: 'flex-end',
        paddingHorizontal: spacing.lg, paddingVertical: spacing.md,
        borderTopWidth: 1, borderTopColor: colors.border,
        backgroundColor: colors.bgDark, gap: spacing.sm,
    },
    inputContainer: {
    ...
    textInput: {
        fontSize: fontSize.base, color: colors.textPrimary, maxHeight: 80,
    },
    sendBtn: {
